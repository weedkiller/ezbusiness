var cntr = {
    ...
    Attachevent: function () {
        ...
        $("#btnAdd").click(function () {
            var n = EzAuthenticationBtn("/FFM_CNTRMaster", "NewIt");
            if (n == 1) {
                $("#tbldiv").hide();
                var abk = [];
        ...
        /*Code Tab Event*/
        EztableTabEve("#tblcntr1", "[name*='txtCNTR_CODE']", "[name *= 'txtNAME']", "Please Enter FNMHEAD_CODE", 'T', "");
        ...
        /*Enter Event*/
        EztableLstEnt("#tblcntr1", "[name*='txtNAME']", "[name *= 'txtCNTR_CODE']", "T", "");


        /*Add Table Change Event*/
        $("#tblcntr1").on("change", "[name*='txtCNTR_CODE']", function () {
            var tr = $(this).closest('tr');

            drec = [];
            $('#tblcntr1 tbody tr td:nth-child(1)').each(function () {
                //add item to array
                var ab = $(this).find("[name *= 'txtCNTR_CODE']").val();
                drec.push(ab);
            });

            drec.splice($.inArray(tr.find("[name *= 'txtCNTR_CODE']").val(), drec), 1);

            if ($.inArray(tr.find("[name *= 'txtCNTR_CODE']").val(), drec) >= 0) {
                var dr = tr.find("[name *= 'txtCNTR_CODE']").val();
                tr.css("background", "red");
                tr.find("[name *= 'txtCNTR_CODE']").focus();
                EzAlerterrtxt("Already Exist Code '" + dr + "'");
                tr.find("[name *= 'txtCNTR_CODE']").val('');
                ...
                if (tr.is(":last-child")) {
                    ...
                    $(this).closest('tr').next('tr').find("[name*='txtCNTR_CODE']").val('');
                    trLast.find("[name *= 'txtCNTR_CODE']").attr({
                        "name": ("txtCNTR_CODE" + counter)
                    });
        ...
        $("#btnCancel").click(function () {
            ...
            var abk1 = [];
            abk1.push("[name*='txtCNTR_CODE']", "[name*='txtNAME']");
            Ezsettxtclr(abk1);
            abk1 = [];
        ...
        $("#btnSave").click(function () {
            ...
            $("#tblcntr1 tbody tr").each(function (index, item) {
                ...
                var CNTR_CODE = $(this).find("[name*='txtCNTR_CODE']").val();
                ...
                if ((CNTR_CODE != "") && (CNTR_NAME != "")) {
                    var FNM_CURRENCYDetail = {
                        FFM_CNTR_CODE: CNTR_CODE,
                        NAME: CNTR_NAME,
                        //MASTER_STATUS:MASTER_STATUS,
                        //Note: NOTE,
                    };
                    Fcur.FFM_CNTRDetailNew.push(FNM_CURRENCYDetail);
                }
            });
            var validationResult = ValidateForm(Fcur);
            if (validationResult.formValid) {
                $.post("/FFM_CNTR/saveFFM_CNTR", Fcur).done(function (response) {
        ...
        function ValidateForm(Fcur) {
            ...
            if (Fcur.FFM_CNTRDetailNew.length == 0) {
        ...
        $("#tblUnits").on("click", "[name^='Edit']", function () {
            var n = EzAuthenticationBtn("/FFM_CNTRMaster", "EditIt");
            if (n == 1) {
                var counter = $("#Counter").val();
                ...
                if (hdnEditMode.val() == "true") {
                    ...
                    var Fcur = {
                        FFM_CNTR_CODE: CURRENCY_CODEEdit,
                ...
                if (hdnMode.val() == "false") {
                    ...
                    swalWithBootstrapButtons.fire({
                        ...
                        if (result.value) {
                            ...
                            $.getJSON("/DeleteFMM_CNTR", { CURRENCYCode: CURRENCY_CODE }).done(function (data) {
                                if (data.DeleteFlag) {

var unit = {
    ...
    Attachevent: function () {
        $("#dcm").hide();
        EzAuthentication("/FrightUnitMaster");
        var n = EzAuthenticationBtn("/FrightUnitMaster", "ViewIt");
        if (n == 1) {
        ...
        $("#btnAdd").click(function () {
            var n = EzAuthenticationBtn("/FrightUnitMaster", "NewIt");
            if (n == 1) {
                $("#tbldiv").hide();
                var abk = [];
        ...
        $("#btnSave").click(function () {
            ...
            $("#tblcntr1 tbody tr").each(function (index, item) {
                ...
                if ((CNTR_CODE != "") && (CNTR_NAME != "")) {
                    var FNM_unitDetail = {
                        FFM_UNIT_CODE: CNTR_CODE,
                        NAME: CNTR_NAME,
                        DISPLAY_STATUS: Displayname
                        //MASTER_STATUS:MASTER_STATUS,
                        //Note: NOTE,
                    };
                    Fcur.FFM_unitDetailsnew.push(FNM_unitDetail);
                }
            });
            var validationResult = ValidateForm(Fcur);
            if (validationResult.formValid) {
                $.post("/FFM_Unit/saveFFM_Unit", Fcur).done(function (response) {
        ...
        function ValidateForm(Fcur) {
            ...
            if (Fcur.FFM_unitDetailsnew.length == 0) {
        ...
        $("#tblUnits").on("click", "[name^='Edit']", function () {
            var n = EzAuthenticationBtn("/FrightUnitMaster", "EditIt");
            if (n == 1) {
                var counter = $("#Counter").val();
                ...
                if (hdnEditMode.val() == "true") {
                    ...
                    var Fcur = {
                        FFM_Unit_CODE: CURRENCY_CODEEdit,
                ...
                if (hdnMode.val() == "false") {
                    ...
                    swalWithBootstrapButtons.fire({
                        ...
                        if (result.value) {
                            ...
                            $.getJSON("/DeleteFMM_Unit", { FFM_Unit_Code: CURRENCY_CODE }).done(function (data) {
                                if (data.DeleteFlag) {

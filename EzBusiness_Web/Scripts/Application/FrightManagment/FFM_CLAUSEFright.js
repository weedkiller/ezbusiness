var clause = {
    ...
    Attachevent: function () {
        $("#dcm").hide();
        EzAuthentication("/FFM_CLAUSEMaster");
        var n = EzAuthenticationBtn("/FFM_CLAUSEMaster", "ViewIt");
        if (n == 1) {
        ...
        $("#btnAdd").click(function () {
            var n = EzAuthenticationBtn("/FFM_CLAUSEMaster", "NewIt");
            if (n == 1) {
                $("#tbldiv").hide();
                var abk = [];
        ...
        /*Code Tab Event*/
        EztableTabEve("#tblcntr1", "[name*='txtCLAUSE_CODE']", "[name *= 'txtNAME']", "Please Enter FNMHEAD_CODE", 'T', "");
        ...
        /*Enter Event*/
        EztableLstEnt("#tblcntr1", "[name*='txtNAME']", "[name *= 'txtCLAUSE_CODE']", "T", "");


        /*Add Table Change Event*/
        $("#tblcntr1").on("change", "[name*='txtCLAUSE_CODE']", function () {
            ...
            if ($.inArray(tr.find("[name *= 'txtCLAUSE_CODE']").val(), drec) >= 0) {
                ...
                if (tr.is(":last-child")) {
                    ...
                    $(this).closest('tr').next('tr').find("[name*='txtCLAUSE_CODE']").val('');
                    trLast.find("[name *= 'txtCLAUSE_CODE']").attr({
                        "name": ("txtCLAUSE_CODE" + counter)
                    });
        ...
        $("#btnCancel").click(function () {
            ...
            var abk1 = [];
            abk1.push("[name*='txtCLAUSE_CODE']", "[name*='txtNAME']");
            Ezsettxtclr(abk1);
            abk1 = [];
        ...
        $("#btnSave").click(function () {
            ...
            $("#tblcntr1 tbody tr").each(function (index, item) {
                ...
                var CLAUSE_NAME = $(this).find("[name*='txtNAME']").val();
                ...
                if ((CLAUSE_CODE != "") && (CLAUSE_NAME != "")) {
                    var FNM_MOVE_Detail = {
                        FFM_CLAUSE_CODE: CLAUSE_CODE,
                        NAME: CLAUSE_NAME,
                        //MASTER_STATUS:MASTER_STATUS,
                        //Note: NOTE,
                    };
                    Fcur.FFM_CLAUSE_Detail.push(FNM_MOVE_Detail);
                }
            });
            var validationResult = ValidateForm(Fcur);
            if (validationResult.formValid) {
                $.post("/FFM_CLAUSE/saveFFM_CLAUSE", Fcur).done(function (response) {
                    ...
                    $("#tblcntr1 tbody tr").each(function (index, item) {
                        ...
                        if (response.Drecord.length > 0) {
                            ...
                            if ($.inArray(CLAUSE_CODE, ary1) < 0 && CLAUSE_CODE != '' && CLAUSE_NAME != '') {
                                $(this).closest('tr').remove();
                                var counter = $("#Counter").val();
                                var firstRow = $('#tblUnits tbody tr:first');

                                var newRow = "<tr><td>" + CLAUSE_CODE + "</td><td>" + CLAUSE_NAME + "</td>";
        ...
        function ValidateForm(Fcur) {
            ...
            if (Fcur.FFM_CLAUSE_Detail.length == 0) {
        ...
        $("#tblUnits").on("click", "[name^='Edit']", function () {
            var n = EzAuthenticationBtn("/FFM_CLAUSEMaster", "EditIt");
            if (n == 1) {
                var counter = $("#Counter").val();
                ...
                if (hdnEditMode.val() == "true") {
                    ...
                    var Fcur = {
                        FFM_CLAUSE_CODE: CURRENCY_CODEEdit,
